...
def train(model, train_loader, eval_loader,args,qid2type):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (v, q, a, b, hintscore,type_mask,notype_mask,q_mask) in tqdm(enumerate(train_loader), ncols=100,
            ...
            if mode=='updn':
                pred, loss,_ ,_= model(v, q, a, b, None)
                if (loss != loss).any():
                ...
                total_loss += loss.data[0] * q.size(0)
                batch_score = compute_score_with_logits(pred['answer'], a.data).sum()
                ...
                if topv==-1:
                    ...
                    for x in range(a.shape[0]):
                        ...
                        v_mask[x].scatter_(0,v_mask_ind[x,:num].data,1)
                else:
                ...
                pred, loss, _ ,_= model(v, q, a2, b, v_mask)
                ...
                if random_num<=qvp:
                    ## first train
                    pred, loss, word_emb,_  = model(v, q, a, b, None)
                    word_grad = torch.autograd.grad((pred['answer'] * (a > 0).float()).sum(), word_emb, create_graph=True)[0]
                    ...
                    q2 = copy.deepcopy(q_mask.data)

                    m1 = copy.deepcopy(sen_mask.data)  ##[0,0,0...0,1,1,1,1]
                    m1.scatter_(1, w_ind.data, 0)  ##[0,0,0...0,0,1,1,0]
                    m2 = 1 - m1  ##[1,1,1...1,1,0,0,1]
                    if dataset=='cpv1':
                        m3=m1*18330
                    else:
                    ...
                    q2 = q2 * m2.long() + m3.long()
                    q2 = Variable(q2)
                    pred, _, _ ,_= model(v, q2, None, b, None)
                    _,pred_ind = torch.sort(pred['answer'], 1, descending=True)
                    pred_ind = pred_ind[:, :5]
                    false_ans = torch.ones(pred['answer'].shape[0], pred['answer'].shape[1]).cuda()
                    false_ans.scatter_(1, pred_ind.data, 0)
                    false_ans = Variable(false_ans).cuda()
                    a2 = a * false_ans
                    q3 = copy.deepcopy(q.data)
                    if dataset=='cpv1':
                        q3.scatter_(1, w_ind.data, 18330)
                    else:
                        q3.scatter_(1, w_ind.data, 18455)

                    ## third train
                    q3 = Variable(q3)
                    pred, loss, _ ,_ = model(v, q3, a2, b, None)
                    ...
                    ## first train
                    pred, loss, _ ,_ = model(v, q, a, b, None)
                    visual_grad = torch.autograd.grad((pred['answer'] * (a > 0).float()).sum(), v, create_graph=True)[0]
                    ...
                    pred, _, _ ,_= model(v, q, None, b, v_mask)
                    _,pred_ind = torch.sort(pred['answer'], 1, descending=True)
                    pred_ind=pred_ind[:, :5]
                    false_ans = torch.ones(pred['answer'].shape[0], pred['answer'].shape[1]).cuda()
                    false_ans.scatter_(1, pred_ind.data, 0)
...
def evaluate(model, dataloader, qid2type):
    ...
    for v, q, a, b, qids, _ in tqdm(dataloader, ncols=100, total=len(dataloader), desc="eval"):
        ...
        batch_score = compute_score_with_logits(pred['answer'], a.cuda()).cpu().numpy().sum(1)
        score += batch_score.sum()
        upper_bound += (a.max(1)[0]).sum()
        qids = qids.cpu().int().numpy()
